...
$(document).ready(function(){
    ...
    $.validate({
        ...
        onSuccess : function($form) {
            if($form.attr('id') === 'ContactForm'){
                 ...
                 return false;
            }else if($form.attr('id') === 'VolunteerForm2'){
                 survialvolunteerformsubmitdetailsjax();
                 return false;
               }
        },
    });
    ...
    function volunteerformsubmitdetailsjax() {
        $.ajax({
            ...
            data: {
                ...
                gender: $('#VolunteerForm select[name="gender"]').val(),
                age: $('#VolunteerForm select[name="age"]').val(),
            },
            success: function(result){
                //clear fields
    ...
    // Submit Survial Backpack volunteer form details
    function survialvolunteerformsubmitdetailsjax() {
        $.ajax({
            url: '/volunteer_form_backpack_submit.php',
            type: 'post',
            data: {
                surname: $('#VolunteerForm2 input[name="surname"]').val(),
                firstname: $('#VolunteerForm2 input[name="firstname"]').val(),
                email: $('#VolunteerForm2 input[name="email"]').val(),
                phone: $('#VolunteerForm2 input[name="phone"]').val(),
                address: $('#VolunteerForm2 textarea[name="address"]').val(),
                gender: $('#VolunteerForm2 select[name="gender"]').val(),
                sponsor: $('#VolunteerForm2 select[name="sponsor"]').val(),
                // why: $('#VolunteerForm textarea[name="why"]').val()
                sponsorName: $('#VolunteerForm2 select[name="sponsorName"]').val(),
                website: $('#VolunteerForm2 input[name="website"]').val(),
                gift: $('#VolunteerForm2 select[name="gift"]').val(),
                giftName: $('#VolunteerForm2 input[name="giftName"]').val()
            },
            success: function(result){
                //clear fields
                $("#VolunteerForm2").trigger('reset');

                //show success msg
                $("#survialvolunteeremailsendresponse").html(result);

                //close modal
                setTimeout(function(){
                    $('#volunteerformModal2').modal('toggle');
                }, 2000);
            }
        });
    }
